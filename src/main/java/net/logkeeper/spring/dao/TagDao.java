...
public class TagDao {
	...
	public List listTagFileGroup(String languagePrefix,Boolean jira) {
		Query query = getCurrentSession().createQuery("from Tag c where c.name like :fileTag and jira= :jira");
		List tagList = query.setParameter("fileTag", "%" + languagePrefix + "%").setMaxResults(10).setParameter("jira", jira).list();
		return tagList;
	}

	public List findFileTag(String fileTag) {
		Query query = getCurrentSession().createQuery("from Tag c where c.name= :fileTag");
	...
	public List listTag() {
		Query query = getCurrentSession().createQuery("Select id,name from Tag order by clickCount desc");
	...
	public List findByTagId(int fileGroupId) {
		Query query = getCurrentSession()
				.createQuery("select t.id,t.name,t.createDate,t.jira from FileGroup fg join fg.tags t where fg.id= :id");
